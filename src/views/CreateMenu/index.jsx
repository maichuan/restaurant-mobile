...
const CreateMenu = ({ navigation }) => {
    ...
    const cancelAdditional = (index) => {
        console.log(index)
        let newoptions = [...options.slice(0, index), ...options.slice(index + 1, options.length)]
                ...
                <KAVContent behavior={Platform.Os == "ios" ? "padding" : "height"} style={{ flex: 1 }}>
                    <Content>
                        ...
                        <BottomPart>
                            ...
                            {showInstruction()}
                            {options.map((item, index) => <AdditionalCard key={index} item={item} index={index} cancelAdditional={cancelAdditional} setAdditionalPending={setAdditionalPending} editOption={editOption} />)}
                        </BottomPart>
                    </Content>
                </KAVContent>
            </Container>
            <CreateBtn><CreateText>Done</CreateText></CreateBtn>
        </>
    )
}
